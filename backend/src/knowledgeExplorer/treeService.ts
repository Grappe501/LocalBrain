...
function buildOverlay(ws: LivingWorkspace | null, mtime: string | null): ExplorerOverlayBadge[] {
  if (!ws) return [];
...
function nodeFromPath(resolved: string, stats: fs.Stats): ExplorerTreeNode {
  ...
  return {
    ...
    size_bytes: stats.isFile() ? stats.size : cached?.size_bytes ?? null,
    mtime: cached?.mtime ?? mtime,
    workspace_id: ws?.workspace_id ?? cached?.workspace_id ?? null,
    workspace_title: ws?.title ?? null,
    overlay: buildOverlay(ws, cached?.mtime ?? mtime),
  };
}
